...
def _li_to_item(li):
    ...
    if dbid >= 0 and mediatype == 'movie':
        data = _fetch_movie_info(movieid=dbid)
        return _rpc_to_item(data)
    elif dbid >= 0 and mediatype == 'tvshow':
        ...
        if 'trailer' not in data:
            data['trailer'] = vid_tag.getTrailer()
        return _rpc_to_item(data)
    ...
    if mediatype == 'season':
        ...
        if show_data:
            ...
            item.info['fallback'] = show_data.get('trailer') # TODO: support fallback
    ...
    if not item.info['unique_id'].get('type') == 'imdb':
        id = vid_tag.getIMDBNumber() or ''
        id_type = 'imdb' if id.lower().startswith('tt') else None
        if id_type or not item.info['unique_id']:
...
def redirect(url, **kwargs):
    ...
    log.info("Found match from reverse lookup: {}".format(matches[0].info['clean_title']))
